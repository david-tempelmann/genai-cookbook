"""Agent configuration schema and validation functions."""
...
class RetrieverSchemaConfig(BaseModel):
    ...
    # If using Databricks vector search with delta sync,
    # this should the column of the delta table that acts as the primary key
    primary_key: str
...
class RetrieverToolConfig(BaseModel):
    ...
    # Prompt template used to format the retrieved information to present to the LLM to help
    # in answering the user's question.  The f-string {chunk_text} and {metadata} can be used.
    chunk_template: str
    ...
    # A description of the documents in the index.  Used by the Agent to determine if this tool
